...
export default async function LeadsPage({
  ...
  // Status filter
  const validStatuses = ["New", "Contacted", "In Progress", "Waiting for Client", "Completed", "Closed"];
  const statusParam = typeof resolvedParams['status'] === "string" ? resolvedParams['status'] : "";
  const statusFilter = validStatuses.includes(statusParam) ? statusParam : null;
  ...
  let query = supabase.from("leads").select("id, full_name, email, service_requested, country, status, created_at", { count: "exact" });
      ...
      <div className="mb-6 rounded-md border border-border bg-surface p-4">
        <form method="get" action="/admin/leads" className="flex flex-wrap gap-4 items-end">
          <div className="grid gap-1">
            <label htmlFor="search" className="text-xs font-bold uppercase text-muted-foreground">Search</label>
            <input 
              type="text" 
              name="search" 
              id="search"
          ...
          <button type="submit" className="h-10 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary-hover">
            Filter
          </button>
        </form>
      </div>
            ...
            {!leads || leads.length === 0 ? (
              ...
              leads.map((lead) => (
                <tr key={lead.id} className="border-b border-border last:border-0 hover:bg-background transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/admin/leads/${lead.id}`} className="text-primary-text hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                      View
                    </Link>
                  </td>
                </tr>
