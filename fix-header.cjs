...
export function SiteHeader({ categories = [] }: { categories?: CategoryWithServices[] }) {
  const pathname = usePathname();

  const columns = categories
    .filter((c) => c.services && c.services.length > 0)
    .map((c) => ({
      title: c.name,
      links: c.services.map((s) => [
        s.title.replace(" Business", ""), // Clean up platform names if needed
        \`/services/\${s.slug}\`,
      ]) as [string, string][],
    }));
...
`;
fs.writeFileSync("src/components/site-header.tsx", content);
